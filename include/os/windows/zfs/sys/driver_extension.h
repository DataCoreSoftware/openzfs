...
/*
 * ExAllocatePoolZero was introduced in WDK 10.0.19041.0.
 * Provide a compatibility shim for older WDK versions.
 */
#ifndef ExAllocatePoolZero
static inline void *
_ExAllocatePoolZero(POOL_TYPE PoolType, SIZE_T NumberOfBytes, ULONG Tag)
{
	void *p = ExAllocatePoolWithTag(PoolType, NumberOfBytes, Tag);
	if (p != NULL)
		RtlZeroMemory(p, NumberOfBytes);
	return (p);
}
#define	ExAllocatePoolZero(PoolType, NumberOfBytes, Tag) 	_ExAllocatePoolZero((PoolType), (NumberOfBytes), (Tag))
#endif /* ExAllocatePoolZero */
