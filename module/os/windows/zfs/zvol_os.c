...
zvol_os_attach(char *name)
{
	...
	if (zv != NULL) {
		...
		if (error == 0) {
			...
			wzvol_announce_buschange();
			dprintf("%s:%d returning zvol '%s', targetid:%d, lun_id:%d, open_count:%d, volsize:%llu, flags:%d\n",
			    __func__, __LINE__, zv->zv_name, zv->zv_zso->zso_target_id, zv->zv_zso->zso_lun_id,
