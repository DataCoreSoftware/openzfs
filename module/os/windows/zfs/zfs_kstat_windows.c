...
/*
 * Stubs for variables that became static in upstream modules or were removed.
 * These kstat entries show/accept values but don't affect the actual module
 * parameters (which are no longer externally accessible).
 */
...
static uint_t zfs_vdev_async_read_max_active = 3;
static uint_t zfs_vdev_async_write_min_active = 1;
/* zfs_vdev_async_write_max_active is a global in vdev_queue.c */

/* arc_kstat_update_windows is #if 0'd in arc_os.c — provide a no-op stub */
int
...
static uint_t zfs_vdev_scrub_max_active = 2;
static int zfs_vdev_async_write_active_min_dirty_percent = 30;
static int zfs_vdev_async_write_active_max_dirty_percent = 60;
static int zfs_vdev_aggregation_limit = 1048576;
static int zfs_vdev_read_gap_limit = 32768;
static int zfs_vdev_write_gap_limit = 4096;
static hrtime_t zfs_delay_max_ns = 100000000;
static unsigned int zfetch_max_streams = 8;
static unsigned int zfetch_min_sec_reap = 2;
static uint64_t metaslab_aliquot = 524288;
static int zfs_vdev_cache_max = 0;		/* removed upstream */
static int spa_max_replication_override = 0;
static int zfs_no_scrub_io = 0;
static int zfs_no_scrub_prefetch = 0;
static ssize_t zfs_immediate_write_sz = 32768;
static ssize_t zvol_immediate_write_sz = 32768;
static int64_t zfs_free_bpobj_enabled = 1;
static int zfs_send_corrupt_data = 0;
static int zfs_send_queue_length = 16777216;
static int zfs_recv_queue_length = 16777216;
static uint64_t zfs_send_set_freerecords_bit = 1;
static uint64_t send_holes_without_birth_time = 1;
static uint64_t dbuf_cache_max_bytes = UINT64_MAX;
static uint64_t zfs_trim_extent_bytes_max = 134217728;
static uint64_t zfs_trim_extent_bytes_min = 32768;
static uint64_t zfs_trim_txg_batch = 8;
static uint64_t zfs_trim_queue_limit = 10;
static uint64_t zfs_send_unmodified_spill_blocks = 1;
static uint64_t zfs_special_class_metadata_reserve_pct = 25;
static uint64_t zfs_initialize_value = 0xdeadbeefdeadbeefULL;
static int zfs_autoimport_disable = 0;
static uint32_t cpu_avx_supported = 0;	/* removed upstream */
...
windows_kstat_update(kstat_t *ksp, int rw)
{
	...
	if (rw == KSTAT_WRITE) {
		...
		/* zfs_prefetch_disable removed upstream */
		zfetch_max_streams =
		...
		/* zfs_vdev_cache_size removed upstream */
		zfs_no_scrub_io =
...
//		    ks->zfs_read_chunk_size.value.i64;
		...
		ks->spa_asize_inflation.value.i64 =
		    spa_asize_inflation;
		ks->zfs_prefetch_disable.value.i64 = 0;
		ks->zfetch_max_streams.value.i64 =
		    zfetch_max_streams;
		ks->zfetch_min_sec_reap.value.i64 =
		    zfetch_min_sec_reap;
		...
		ks->zfs_vdev_cache_max.value.i64 =
		    zfs_vdev_cache_max;
		ks->zfs_vdev_cache_size.value.i64 = 0;
		ks->zfs_no_scrub_io.value.i64 =
		    zfs_no_scrub_io;
		ks->zfs_no_scrub_prefetch.value.i64 =
		    zfs_no_scrub_prefetch;
...
kstat_windows_init(void *arg)
{
	...
	if (windows_kstat_ksp != NULL) {
		...
		/* spl_kstat_registry removed upstream — skip registry override */
