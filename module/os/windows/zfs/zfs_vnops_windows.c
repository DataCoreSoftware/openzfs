	...
	if (IsEqualGUID(IrpSp->Parameters.QueryInterface.InterfaceType,
		...
		if (IrpSp->Parameters.QueryInterface.Version < 1)
			...
			if (zv) {
				...
				TraceEvent(TRACE_INFO, "%s:%d: query DI interface for zvol:%s, size:%llu, lun_id:%d, target_id:%d",
				    __func__, __LINE__, pzv->zv_name, pzv->zv_volsize,
				    pzv->zv_zso->zso_lun_id, pzv->zv_zso->zso_target_id);
			}
